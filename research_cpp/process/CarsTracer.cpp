...
namespace ImgProc
{
	...
	{
		auto labelNum = cv::connectedComponentsWithStats(mTemp, mLabels, mStats, mCentroids); // ラベリング
	...
	void CarsTracer::TraceCars(const size_t& idx)
	{
		...
		for (auto carId = Tk::sFrontCarsId; carId < Tk::sCarsNum; carId++)
		{
			...
			MatchByLabels(carPos);
			/* end */
		}
		/* end */
	}

	template<class T> void CarsTracer::MatchByLabels(cv::Rect_<T>& carPos)
	{
		const auto& label = mLabels.at<int>(carPos.y, carPos.x);
	}
